<?php
    ...
    if(isset($_POST['EDIT'])){
        ...
        if($authen != $authena){
            ...
            $sql = "UPDATE pay_package SET p_name = '$pname', p_price = '$pprice', p_desc = '$desc' WHERE p_id='$pid'";
        ...
        header('Location: pay_modify_package.php');
    }else{
        $authen = '';
    }
    ...
    if(isset($_GET['pid'])){
        ...
        $r = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM pay_package WHERE p_id='$pid'"));
        if(count($r) != 0){
    ...
    <body class="w3-padding-0 w3-margin-0">
        ...
        <div class="w3-left" style="cursor: pointer;"> <a href="homepage.php"><i class="fa fa-chevron-left"></i>BACK</a></div>
            ...
            <form method="post" enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF'];?>" class="w3-content w3-card-12">
                ...
                <div class="w3-margin">
                    <input type="text" class="w3-input" name="p_name" placeholder="Enter Package Name" value="<?php echo $r['p_name'];?>" required>
                    <label class="w3-label w3-validate">Package Name</label>
                </div>
                
                <div class="w3-margin">
                    <input type="text" class="w3-input" name="p_price" placeholder="Enter Package Price" value="<?php echo $r['p_price'];?>" required>
                    <label class="w3-label w3-validate">Package Price</label>
                </div>
                
                <div class="w3-margin">
                    <textarea class="w3-input" name="desc" class="custom-control" placeholder="Enter Package Description" required><?php echo $r['p_desc'];?></textarea>
...
<!--                 
                ...
                <div class="w3-margin">
                    <input class="w3-btn-block" type="submit" name="EDIT" value="EDIT">
                </div>
