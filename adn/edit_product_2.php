<?php
    $msg1 = $msg2 = $authen = "...";
    session_start();
    include("../include/connectdb.php");
    $p = new db();
    $con = $p->con(); 
    
    //print_r($_POST);
    
    include("config.php");
    if(isset($_COOKIE[md5("admin_signed_in")])){
        $authena = base64_decode($_COOKIE[md5("admin_authen")]);
    }else{
        ?>
            <script type="text/javascript">
                window.location.replace("index.php");
            </script>
        <?php
    }
    
    if(isset($_GET['id'])){
        $pid = $p->sqlr($con,$p->xssr($_GET['id']));
        $r = mysqli_fetch_assoc($sql = mysqli_query($con,"SELECT * FROM product WHERE p_id='$pid'"));
        
    }else if(isset($_POST['pid'])){
       
        $pid = $p->sqlr($con,$p->xssr($_POST['pid']));
        
        if(isset($_POST['edit_text'])){
            $pname = $p->sqlr($con,$p->xssr($_POST['p_name']));
            $product_type = $p->sqlr($con,$p->xssr($_POST['product_type']));
            $pprice = $p->sqlr($con,$p->xssr($_POST['p_price']));
            $desc = $p->sqlr($con,($_POST['desc']));
            $p_content = $p->sqlr($con,($_POST['p_content']));
            //$authen = $p->sqlr($con,$p->xssr($_POST['authen']));
            
            //print_r($_POST);
            //exit();
            
            if($authen != $authena){
                $msg1 = "modification key is invalid";
            }else{
                $sql = "UPDATE product SET p_name='$pname',p_price='$pprice',p_desc='$desc',p_content='$p_content', p_type='$product_type' WHERE p_id='$pid'";
                if(mysqli_query($con,$sql)){
                    $msg1 = "Data Updated Successfully";
                    ?>
                            <script type="text/javascript">
                                alert("EDITED Successfully");
                            </script>
                       <?php
                }else{
                    $msg1 = "Error Updating Data";
                }
            }
            
        }else if(isset($_POST['edit_image'])){
            //$authen = $p->sqlr($con,$p->xssr($_POST['authen']));
            if($authen != $authena){
                $msg2 = "modification key is invalid.";
            }else{
                $pix = $_FILES['p_image'];
                
                //get old image link
                $rold = mysqli_fetch_assoc(mysqli_query($con,"SELECT * FROM product WHERE p_id='$pid'"));
                $old_image_path_server = "../media/image/product/".$rold['p_image'];
                
                //check validity
                if(getimagesize($pix['tmp_name'])){
                    //upload to server
                    
                    $path_name = "product_".rand(1,10000).basename($pix['name']);
                    $path_server = "../media/image/product/".$path_name;
                    
                    if(move_uploaded_file($pix['tmp_name'], $path_server)){
                        $sql = "UPDATE product SET p_image='$path_name' WHERE p_id='$pid'";
                        if(mysqli_query($con,$sql)){
                           $msg2 = "Successfully Edited Product image";
                           unlink($old_image_path_server);
                           ?>
                            <script type="text/javascript">
                                alert("EDITED Successfully");
                            </script>
                       <?php
                        }else{
                           $msg2 = "DB ERROR"; 
                           unlink($path_server);
                        }
                    }else{
                        $msg2 = "server error";
                    }
                    
                }else{
                    $msg2 = "File Uploaded is not a valid image";
                }
            }
        }
        
        
        //last query
        $r = mysqli_fetch_assoc($sql = mysqli_query($con,"SELECT * FROM product WHERE p_id='$pid'"));
    }
    else{
        header('Location:edit_product.php');
    }
    
    
?>

<!DOCTYPE HTML>
<html>
    <head>
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <meta http-equiv="Cache-Control" content="no-cache">
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8">           
        <title>FEMQUEN And Co. Int'l</title>
        <link rel="stylesheet" href="../template/lib/w3.css">         
        <link rel="stylesheet" href="../template/ckec/samples/css/samples.css">
        
        <link rel="stylesheet" href="../template/font-awesome/css/font-awesome.min.css">
        
        <link rel="icon" href="../media/image/icon/icon.png" type="image/x-icon">
        <link rel="stylesheet" type="text/css" href="../template/css/index.css">
        <script type="text/javascript" src="../template/js/jquerym.js"></script>
        <script type="text/javascript" src="../template/js/index.js"></script>
        
        <script type="text/javascript" src="../template/ckec/ckeditor.js"></script>
        <script type="text/javascript" src="../template/ckec/samples/js/sample.js"></script>
    </head>
    <body class="w3-padding-0 w3-margin-0">
        <div id='body' class="w3-container w3-padding-0 w3-margin-0">
        
            <button class="w3-btn-block" onclick="window.location.href='edit_product.php'"> BACK TO LIST</button>
            
            <form name="data" method="post" enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF'];?>" class="w3-content w3-card-12" onsubmit="reprocess()">
                <h2 class="w3-center w3-card-4">ADD A PRODUCT</h2>
                
                <input type="hidden" name="pid" value="<?php echo $r['p_id'];?>">
                
                
                <div class="w3-margin">
                    <input type="text" class="w3-input" name="p_name" value="<?php echo $r['p_name'];?>" placeholder="Enter Product Name" required>
                    <label class="w3-label w3-validate">Product Name</label>
                </div>
                
                <div class="w3-margin">
                    <input type="text" class="w3-input" name="p_price" value="<?php echo $r['p_price'];?>" placeholder="Enter Product Price" required>
                    <label class="w3-label w3-validate">Product Price</label>
                </div>
                
                <div class="w3-margin">
                    <textarea class="w3-input" style="height: 400px" name="desc" placeholder="Enter Product Description" required><?php echo $r['p_desc'];?></textarea>
                    <label class="w3-label w3-validate">Product Description</label>
                </div>
                
                <br>
                <div class="w3-margin">
                    <select name="product_type" class="w3-input w3-round w3-padding w3-border">
                        <option value="0">Product</option>
                        <option value="1">Machine</option>
                    </select>
                    <label class="w3-label w3-validate">Category</label>
                </div>
                
                <div class="w3-margin w3-container">
                    <textarea class="w3-input w3-round" style="height:400px; display: none;" name="p_content" placeholder="Enter Product Explanation"><?php echo $r['p_content'];?></textarea>
                    
                    <div id="editor">
                        <?php echo $r['p_content'];?>
                    </div>
                    <label class="w3-label w3-validate">Product Explanation/Content</label>
                </div>
                
                <div class="w3-margin"><?php echo $msg1;?></div>
                
                <div class="w3-margin">
                    <input class="w3-btn-block" type="submit" name="edit_text" value="EDIT DATA">
                </div>
                
                <br>
            </form>
            
            <form name="image" method="post" enctype="multipart/form-data" action="<?php echo $_SERVER['PHP_SELF'];?>" class="w3-content w3-card-12">
            
                <input type="hidden" name="pid" value="<?php echo $r['p_id'];?>">
                
                <br>
                <div class="w3-card-12" style="position: relative; left:15%; width: 70%;">
                    <img src="../media/image/product/<?php echo $r['p_image'];?>" style="width: 100%; height: 350px;">
                </div>
                <br>
            
                <div class="w3-margin">
                    <input type="file" class="w3-input" name="p_image" required>
                    <label class="w3-label w3-validate">Product Image</label>
                </div>
                
                
                <div class="w3-margin"><?php echo $msg2;?></div>
                
                <div class="w3-margin">
                    <input class="w3-btn-block" type="submit" name="edit_image" value="EDIT IMAGE">
                </div>
                
            </form>
                        
        </div>
        
        <script>
                initSample();
                
                function reprocess(){
                    $("textarea[name='p_content']").html( CKEDITOR.instances['editor'].getData() );
                    return true;
                }
        </script>
    </body>
</html>
