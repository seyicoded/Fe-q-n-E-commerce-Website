    ...
    <body class="w3-padding-0 w3-margin-0">
        ...
        <div>
            <table class="w3-table w3-bordered w3-striped w3-border w3-hoverable">
                ...
                <tbody>
                <?php
                    $dt = mysqli_query($con,"SELECT * FROM post ORDER BY p_id DESC");
                    while($r = mysqli_fetch_assoc($dt)){
                        ?>
                            <tr>  
                                <td><?php echo $r['p_id']?></td>
                                <td><?php echo ((intval($r['user_id']) == 0) ? 'ADMIN': 
                                (mysqli_fetch_assoc(mysqli_query($con, "SELECT * FROM pay_users WHERE pu_id='".$r['user_id']."'"))['pu_fullname']))?></td>
                                <td><?php echo $r['p_title']?></td>
                                <td><?php echo $r['p_type']?></td>
                                <td><?php echo $r['p_desc']?></td>
                                <td><?php echo date("Y-m-d @ H:i a",strtotime($r['date']))?></td>
                                <td><?php echo ((intval($r['status']) == 1) ? 'ACTIVE':'IN-ACTIVE')?></td>
                                <td><a href="pay_modify_post.php?pid=<?php echo $r['p_id'];?>" class="w3-btn w3-green w3-text-white w3-round">EDIT</a></td>
                                <td><a href="pay_delete_post.php?pid=<?php echo $r['p_id'];?>" class="w3-btn w3-red w3-text-white w3-round">DELETE</a></td>
                            </tr>
                        <?php
                    }
                ?>
